...
*/
{
    for (int i = 1; i < n; i++) {
		...
		for (int j = n - 1; j >= i; j--)
			divTable[j][i] = divTable[j][i - 1] - divTable[j - 1][i - 1];
	}
}
...
{
    float sum = divTable[n-1][0];/*initialize f(x)=f(x0)*/
    float u = (xi - f[n - 1].x) / (f[1].x - f[0].x);

    for (int i = 1; i < n; i++)
    {
        sum = sum + (u_cal(u, i) * divTable[n-1][i])/fact(i) ;/*applying the formula*/
    }
    return sum;
}
